"use client";
// Port of Whatsapp/client/src/components/admin/TenantPanel.jsx
...
export default function TenantPanel({
  ...
  const planLine = [
    ...
    t.nextBillingDate ? `חיוב: ${new Date(t.nextBillingDate).toLocaleDateString("he-IL", { day: "2-digit", month: "2-digit", year: "2-digit" })}` : null,
  ]
    .filter(Boolean)
    .join(" · ");
    ...
    <div className="flex flex-col h-full bg-slate-50">
      <div className="bg-white border-b border-slate-200 px-6 pt-5 pb-4 flex-shrink-0">
        <div className="flex items-start gap-4">
          <div className={`w-12 h-12 rounded-xl ${col} flex items-center justify-center text-white font-bold text-lg flex-shrink-0`}>{t.name.charAt(0).toUpperCase()}</div>
        ...
        <div className="flex items-center gap-2 mt-4 justify-end flex-wrap">
          {t.waStatus === "connected" && (
            <button onClick={onCompose} className="flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold px-4 py-2 rounded-lg transition shadow-sm">
          ...
          {TABS.map((tab) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-3.5 text-sm font-medium whitespace-nowrap border-b-2 transition ${
                activeTab === tab.key ? "border-indigo-500 text-indigo-600" : `border-transparent hover:text-slate-700 hover:border-slate-300 ${tab.key === "security" ? "text-red-400" : "text-slate-500"}`
              }`}
            >
              <span className="text-base leading-none">{tab.icon}</span>
              {tab.label}
              {tab.key === "notes" && t.billingStatus === "overdue" && <span className="w-1.5 h-1.5 rounded-full bg-red-500 flex-shrink-0" />}
            </button>
          ))}
        </div>
      </div>
