...
function ClientRow({ client: c, selected, onClick }: { client: any; selected: boolean; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-right flex items-center gap-3 px-4 py-3.5 border-b border-slate-100 transition-all ${selected ? "bg-purple-50 border-r-2 border-r-purple-500" : "hover:bg-slate-50"}`}
    >
      ...
      <div className="flex-1 min-w-0 text-right">
        ...
        <div className="flex items-center justify-end gap-2 mt-0.5">
          ...
          <span className="text-xs text-slate-400 truncate">{managed > 0 ? `${managed} קבוצות` : c.phone}</span>
        </div>
      </div>
    </button>
  );
    ...
    <aside className="w-72 flex-shrink-0 flex flex-col bg-white border-l border-slate-200 shadow-sm" style={{ direction: "rtl" }}>
      <div className="px-4 pt-5 pb-3 border-b border-slate-100">
        <div className="flex items-center justify-between mb-3">
          <button onClick={onAdd} className="flex items-center gap-1.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-medium px-3 py-1.5 rounded-lg transition">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-slate-50 border border-slate-200 rounded-lg pr-9 pl-3 py-2 text-sm text-slate-800 placeholder-slate-400 outline-none focus:ring-2 focus:ring-purple-500/30 focus:border-purple-400 transition"
          />
        </div>
      </div>
      ...
      <div className="px-3 py-2 border-b border-slate-100 flex gap-1 overflow-x-auto flex-shrink-0">
        {FILTERS.map((f) => (
          <button
            ...
            className={`flex-shrink-0 text-[11px] font-medium px-2.5 py-1 rounded-full transition ${activeFilter === f.key ? "bg-purple-600 text-white" : "text-slate-500 hover:bg-slate-100"}`}
          >
            {f.label}
